...
def get_random_sharp_frames(fn_seq, window_size=5, avg_frames=1, dark_tresh=8,
                            number_of_picked_frames=20, max_attemps=100, rng=None):
    ...
    try:
        for fn in fn_seq:
            ...
            for frameId in frameIdxs:
                ...
                valid_frame = True
                attemps += 1
                #print "number_of"+str(number_of_picked_frames)
                ...
                for _ in xrange(window_size):
                    ...
                    for c, a in enumerate(xrange(-avg_frames + 1, avg_frames)):
                        ...
                        #if show1time:
                        #    show1time = False
                        #    cv2.imwrite('blur.jpp',curent_blurry[:, :, [2, 1, 0]])
                        #    cv2.imwrite('sharp.jpg',current_sharp[:, :, [2, 1, 0]])
                    ...
                    if valid_frame:
                        #print len(subframes)
                        #print np.array(subframes).mean(axis=0).shape
                        #print subframes[0].shape
                        #test = np.array(subframes).mean(axis=0) - np.array(subframes)
                        #print test.shape
                        #print np.array(test).sum()
